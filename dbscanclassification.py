...
def main(root_dir: str):
    ...
    # Use 10-fold to get Fold 7
    skf = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
    for fold, (train_idx, test_idx) in enumerate(skf.split(X_scaled, y)):
        if fold == 6:  # Fold 7 (0-based index: 6)
            X_train, X_test = X_scaled[train_idx], X_scaled[test_idx]
            y_train, y_test = y[train_idx], y[test_idx]
            break
    
    # Train and evaluate on Fold 7
    rf_clf.fit(X_train, y_train)
    y_pred = rf_clf.predict(X_test)
    fold7_accuracy = (y_pred == y_test).mean()
    fold7_report = classification_report(y_test, y_pred, output_dict=True)
    print(f"\nFold 7 Accuracy: {fold7_accuracy:.4f}")
    print(f"Fold 7 Classification Report:")
    print(classification_report(y_test, y_pred))
    
    # Save model, scaler, and geometry
    joblib.dump(rf_clf, os.path.join(MODEL_DIR, 'rf_model_fold7.pkl'))
    ...
    with open(os.path.join(OUTPUT_DIR, 'metrics.txt'), 'w') as f:
        f.write(f"Fold 7 Accuracy: {fold7_accuracy:.4f}\n")
        f.write("\nFold 7 Classification Report:\n")
        for cls in ['0', '1']:
            f.write(f"Class {cls}: Precision={fold7_report[cls]['precision']:.2f}, "
                    f"Recall={fold7_report[cls]['recall']:.2f}, F1={fold7_report[cls]['f1-score']:.2f}\n")
        f.write("\nFull Data Confusion Matrix:\n")
